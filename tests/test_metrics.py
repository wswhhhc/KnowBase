...
from src.metrics_dashboard import _quality_fail_rate
...
class MetricsDashboardTests(unittest.TestCase):
    def test_quality_fail_rate_uses_recent_n_rows(self):
        ...
        self.assertEqual(_quality_fail_rate(df), 50.0)
        self.assertEqual(_quality_fail_rate(df, recent_n=2), 0.0)
        self.assertEqual(_quality_fail_rate(df, recent_n=3), 33.333333333333336)
