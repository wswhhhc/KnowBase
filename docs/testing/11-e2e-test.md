...
仓库现在已经接入 Playwright 基础设施，并覆盖了准生产团队版的关键 E2E 权限流：
...
- `editor` 可上传文档、等待导入任务完成、在聊天页问答，并跳转到引用来源
...
当前基础设施已经覆盖登录、管理、成员授权、导入任务、问答、来源跳转和基础权限拒绝。后续可以继续增量补齐这些场景：

1. 登录态刷新与未登录重定向的跨刷新持久化验证
2. 后台任务失败、重试和取消的端到端错误态展示
