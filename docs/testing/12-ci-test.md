...
- 安装 Python 3.12 与 `uv`
- 启动隔离的 Postgres 16 service，数据库名为 `knowbase_test`
- 在 `backend/` 工作目录执行 `uv sync`
- 设置 `KNOWBASE_TEST_POSTGRES_URL` 后运行 `uv run pytest tests --tb=short -q`
- 在真实 Postgres 上验证 Chat 首次建会话竞态、跨工作区冲突、工作区迁移等待和 persist/delete 锁协同
- 在仓库根目录运行 `python scripts/check-structure.py`
...
提交前建议先执行仓库的一键非 E2E 门禁：

```bash
bash scripts/run-checks.sh
```

该脚本覆盖后端测试、结构守卫、前端测试、生产构建和 API 类型漂移检查，但不会启动 E2E 所需的后端、前端与浏览器环境。也可以按下面方式分步执行相同检查：
...
未设置 `KNOWBASE_TEST_POSTGRES_URL` 时，本地后端套件会跳过 4 个真实 Postgres 并发测试；GitHub CI 会启动隔离的 Postgres 16 并强制执行。为防止误清理其他数据库，本地也只接受专用数据库名 `knowbase_test`：

```bash
(cd backend && KNOWBASE_TEST_POSTGRES_URL=postgresql+psycopg://user@localhost:5432/knowbase_test uv run pytest tests/test_chat_turn_postgres.py -q)
```

若要在本地完整模拟 GitHub CI，还需单独执行：
