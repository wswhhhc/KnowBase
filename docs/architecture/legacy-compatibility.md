...
|---|---|---|---|---|---|
| Legacy API Key | 后端认证与前端 API 客户端维护者 | `frontend/src/shared/api/client.ts` 在无 JWT 时读取 `knowbase_api_key`；`frontend/src/pages/settings/SettingsPage.tsx` 保存该值；`backend/src/api/deps.py` 的 `get_current_user_or_legacy_api_key`、工作区/设置/指标/任务路由保留兼容依赖。后端在 `settings.is_production` 时明确拒绝 API Key，因此仅限本地开发兼容。 | 统计或人工确认本地部署均已迁移到 JWT 登录；移除设置页的本地 Key 写入；将受影响路由改为仅 JWT；更新前端认证、设置页和路由测试。 | 迁移期保留 JWT 会话；若发布后本地开发认证受阻，回滚该独立删除提交，不回滚 RBAC/JWT 数据。 | 不早于 v2.0.0 |
