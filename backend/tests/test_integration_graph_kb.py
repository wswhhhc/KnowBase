...
class GraphKBIntegrationTests(unittest.TestCase):
    def test_retrieve_docs_returns_retrieval_results_from_kb(self):
        ...
        with patch("src.graph._get_llm") as mock_llm_factory:
            mock_llm = MagicMock()
            mock_llm.invoke.return_value = FakeResponse("mock answer")
            mock_llm_factory.return_value = mock_llm
            with patch("src.graph.route_question", return_value={"question_type": "knowledge_base", "search_filter": {}}):
                result = run_query(
    ...
    def test_quality_check_expand_retrieval_increases_kb_calls(self):
        ...
        with patch("src.graph._get_llm", return_value=fake_llm):
            with patch("src.graph.route_question", return_value={"question_type": "knowledge_base", "search_filter": {}}):
                with patch("src.graph.ENABLE_QUALITY_CHECK", True):
                    with patch("src.graph.MAX_RETRIES", 3):
