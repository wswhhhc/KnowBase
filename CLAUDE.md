...
用户提问 → problem routing (LLM+正则)
  ...
  → SSE 流式返回 (event_generator → ChatStreamService)
```
...
- **搜索策略**: `fast`(无 rerank)、`balanced`(条件 rerank)、`high_quality`(必 rerank)、`deep`(扩检索)，偏好存 `localStorage`
- **SSE 流**: `backend/src/api/chat_stream_service.py`（ChatStreamService），`chat.py` 路由仅 27 行
...
| 层 | 框架 | 文件 | 用例 | 策略 |
|---|---|---|---|---|
| 后端 | unittest | 30+ | 441 | LLM mock(FakeLLM)，Chroma patch，SQLite tempdir 隔离。SSE 类型漂移检测、AST 签名校验 |
| 前端 | vitest + @testing-library/react | 22 | 195 | ReadableStream 模拟 SSE (含 CRLF)，mock 数据在 `src/test/mocks/data.ts`。hooks/组件/API/类型漂移全覆盖 |
...
| 文件 | 职责 |
|---|---|
| `api/chat_stream_service.py` | SSE 流编排（320 行，替代原 160 行 event_generator 闭包） |
...
| `components/ChatArea.tsx` | 对话页（搜索策略按钮 + localStorage 持久化 + 键盘导航 radiogroup） |
| `components/MessageBubble.tsx` | 消息气泡（引用编号、证据标签、收藏/反馈/复制/导出） |
